...
public class AddExpenseCategoryAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    final int EMPTY_VIEW = 77777;
    private final Activity mActivity;
    ArrayList<Category> mList;

    public AddExpenseCategoryAdapter(Activity activity, ArrayList<Category> arrayList) {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder viewHolder, int i) {
        ...
        if (getItemViewType(i) == 77777) {
            ((EmptyViewHolder) viewHolder).tvAlertMessage.setText(this.mActivity.getResources().getString(R.string.no_category_added));
            return;
        }
        ...
        myViewHolder.ivDelete.setTag(i);
        myViewHolder.ivDelete.setOnClickListener(view -> {
            AddExpenseCategoryAdapter.this.mList.remove(((Integer) view.getTag()).intValue());
            AddExpenseCategoryAdapter.this.notifyDataSetChanged();
        });
    }
